...
{ 
	AAS_profile= "AAS_bis_fnc_saveInventory_west_data";
};

if( playerSide == east ) then 
{
	AAS_profile= "AAS_bis_fnc_saveInventory_east_data";
};


fnc_ADD_ARSENAL_content=
{
	//Add content to ammo box
	...
	_class= S3_PlayerData select 0;
//	systemchat ('ARSENAL MSG: '+ str _class); //стринг
	

	_items= ['FirstAidKit','ItemCompass','ItemGPS','ItemRadio','ItemMap','Binocular'];
	
	if !(_class isEqualto 100) then
	{
		{
			...
			switch (_type select 0) do 
			{
				...
				{ 
					...
					//_w= _x; { _items pushBack _x; } forEach (_w call bis_fnc_compatibleItems);
					
				};
				case ('Magazine'): { _magazines pushBack _x };
				case ('Item'): { _items pushBack _x };
				case ('Equipment'):{  if (_type select 1 isEqualTo 'Backpack') then { _backpacks pushBack _x } else {_items pushBack _x };
				};
				case ('Mine'): {_magazines pushBack _x };
			};
		} forEach (S3_JrAAS_RULES select _class select 1 select (count (S3_JrAAS_RULES select _class select 1)-1));
	};
...
///////////////////////////////////////////////////////////////////////////////////////////////////
//[ "backpack", "item", "magazine", "weapon" ];

//[VIRTUAL_BOX, AAS_backpacks, false, false ] call BIS_fnc_addVirtualBackpackCargo;
//[VIRTUAL_BOX, AAS_items, false, false ] call BIS_fnc_addVirtualItemCargo;
//[VIRTUAL_BOX, AAS_magazines, false, false ] call BIS_fnc_addVirtualMagazineCargo;
//[VIRTUAL_BOX, AAS_weapons, false, false ] call BIS_fnc_addVirtualWeaponCargo;
/*
AAS_arsenal_content= [AAS_backpacks, AAS_items, AAS_magazines, AAS_weapons];
{
	_toAdd = AAS_arsenal_content select _forEachIndex;
	_fnc = missionNamespace getVariable format [ "BIS_fnc_addVirtual%1Cargo", _x ];
	[VIRTUAL_BOX, _toAdd, false, false ] call _fnc;
...
systemChat Localize 'STR_S3_Magazines_removed';
};
...
AAS_fncLoadSave= 
{
	...
	_hideTemplate = true;
		...
		['showMessage',[_display,format [Localize 'STR_S3_AAS_loadout', ctrltext _ctrlTemplateName]]] call bis_fnc_arsenal;
		...
		if (ctrlenabled _ctrlTemplateName) then {
			...
			if ((_ctrlTemplateValue lbvalue lnbcurselrow _ctrlTemplateValue) >= 0) then {
				...
				[_center,[profilenamespace,_inventory]] call AAS_fnc_loadinventory;
				_center switchmove "";

				//--- Load custom data
				_ctrlTemplateValue = _display displayctrl IDC_RSCDISPLAYARSENAL_TEMPLATE_VALUENAME;
				_data = profilenamespace getvariable [AAS_profile,[]];
		...
		if ({typename _x != typename ""} count _cfg == 0) then {
			...
			if (count _cfg == 1) then {
				...
				if (_nameID >= 0) then {
					...
					["Inventory '%1' not found",_name] call bis_fnc_error; breakout _fnc_scriptName; // стринг
				};
			};
		};
	};
...
if !("uniform" in _blacklist) then {
	...
	if (_uniform != "") then {
		if (isclass (configfile >> "cfgWeapons" >> _uniform)) then {
			...
			["Uniform '%1' does not exist in CfgWeapons",_uniform] call bis_fnc_error; //стринг
		};
	};
};
if !("vest" in _blacklist) then {
	if (_vest != "") then {
		if (isclass (configfile >> "cfgWeapons" >> _vest)) then {
			...
			["Vest '%1' does not exist in CfgWeapons",_vest] call bis_fnc_error; //стринг
		};
	};
};
if !("headgear" in _blacklist) then {
	if (_headgear != "") then {
		if (isclass (configfile >> "cfgWeapons" >> _headgear)) then {
			...
			["Headgear '%1' does not exist in CfgWeapons",_headgear] call bis_fnc_error; //стринг
		};
	};
};
if !("goggles" in _blacklist) then {
	if (_goggles != "") then {
		if (isclass (configfile >> "cfgGlasses" >> _goggles)) then {
			...
			["Goggles '%1' does not exist in CfgGlasses",_goggles] call bis_fnc_error; //стринг
		};
	};
};
if !("backpack" in _blacklist) then {
	...
	if (_backpack == "") then {
		...
		if (isclass (configfile >> "cfgVehicles" >> _backpack)) then {
			...
			["Backpack '%1' does not exist in CfgVehicles",_backpack] call bis_fnc_error; //стринг
		};
	};
};
...
AAS_fnc_saveInventory=
{
//systemchat 'AAS_fnc_saveInventory';
/*
	Author: Karel Moricky
...
{
	_display = _this select 0;
	[_display] call AAS_fnc_showTemplates;
...
AAS_fnc_buttonTemplateDelete=
{
		...
		[_center,[profilenamespace,_name],nil,true] call AAS_fnc_saveInventory;
		[_display] call AAS_fnc_showTemplates;
		_ctrlTemplateValue lnbsetcurselrow (_cursel max (lbsize _ctrlTemplateValue - 1));
		["templateSelChanged",[_display]] call bis_fnc_arsenal;
};
	
AAS_FakeArsenal=true;
...
[ missionNamespace, "arsenalOpened", 
{
	...
	_c ctrlSetText (localize 'STR_S3_Save'+ctrlText _c); 
	_c ctrlSetBackgroundColor [0.6, 1, 0, 0.5 ];
	_c ctrlSetTextColor [0, 0, 0, 1];
	/* _c ctrladdeventhandler ["buttonclick", "[ctrlparent (_this select 0)] call AAS_fnc_showTemplates; [ctrlparent (_this select 0)]  call AAS_fnc_buttonSave;"]; */
	_c ctrlAddEventHandler ["buttonclick", { call S3_fnc_SaveCustomLoadout; }];
	...
	//_c ctrlSetTextColor [99, 99, 99, 1];
	//_c ctrladdeventhandler ["buttonclick","[ctrlparent (_this select 0)] call AAS_fnc_showTemplates; [ctrlparent (_this select 0)] call AAS_fnc_buttonLoad;"];
	...
	_c ctrlSetTextColor [0, 0, 0, 1];
	_c ctrladdeventhandler ["buttonclick","[ctrlparent (_this select 0)] call AAS_fncLoadSave; "];
	
	_c = _display displayctrl IDC_RSCDISPLAYARSENAL_TEMPLATE_BUTTONDELETE;
	_c ctrlRemoveAllEventHandlers "buttonclick";	
	_c ctrladdeventhandler ["buttonclick","[ctrlparent (_this select 0)] call AAS_fnc_buttonTemplateDelete;"];
	...
	_c ctrlAddEventHandler ["buttonclick", { call AAS_RemoveAllMagazines}];
	/* uiNamespace setvariable['ArsenalDSP', _display];
	_c ctrlAddEventHandler ["buttonclick", { copytoclipboard str (getunitloadout player);  ['showMessage',[uiNamespace getvariable['ArsenalDSP',displayNull],'Copied to clipboard']] call bis_fnc_arsenal; }]; */
	_c ctrlSetText Localize 'STR_S3_Clear_M'; 
