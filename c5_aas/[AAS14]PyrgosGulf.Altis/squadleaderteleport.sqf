...
if (flag13 distance (leader player) <= RULES_SquadLeaderToFlagDist)then 
	 { hint format["%1,Your squad leader %2 is too close to zone Lima (%3m remaining)!",Player call getName,_leader call getName,floor (_leader distance flag13)- RULES_SquadLeaderToFlagDist];
	breakTo "exit";};
if (flag14 distance (leader player) <= RULES_SquadLeaderToFlagDist)then 
	 { hint format["%1,Your squad leader %2 is too close to zone Mike (%3m remaining)!",Player call getName,_leader call getName,floor (_leader distance flag14)- RULES_SquadLeaderToFlagDist];
	breakTo "exit";};
if (flag15 distance (leader player) <= RULES_SquadLeaderToFlagDist)then 
	 { hint format["%1,Your squad leader %2 is too close to zone November (%3m remaining)!",Player call getName,_leader call getName,floor (_leader distance flag15)- RULES_SquadLeaderToFlagDist];
	breakTo "exit";};	 		 		  
