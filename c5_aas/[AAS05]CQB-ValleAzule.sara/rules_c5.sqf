...
//RULES FOR C5 - MODIFIED FROM DEFAULT RULES
RULES_ruleSetName = "C5";
...
RULES_classChangeDelay = 0;
RULES_banScopedWithHeavy = false;
RULES_defaultTagDisplayRange = 1500;   // range at which green player tags disppear (too far away to show)
...
RULES_captureRadius = 3;      //the radius of the flag that you need to get within to capture fully decamped base
RULES_captureSpeedFactor = .75;  //the capture speed factor multiplier (1=normal speed, 2=double speed, 0.5=half speed etc...)
...
RULES_spawnArmourDuration = 5;     // how long does spawn armour last (if you go below 5secs armour will disable or when you pull your gun trigger)
RULES_spawncampRadius         = 50	; 	// defines the allowed range to walk within enemy base without punishment
RULES_spawncampMinLifespan    = 10 	;  // minimum lifespan under which assumed spawncamper has killed you
...
RULES_overrideVehicleSpawnTimes = false;  // determines whether or not we should use rule settings for vehicle respawn
...
RULES_secondsDefzoneToScore  = 30 ;       // time period for defending zone
