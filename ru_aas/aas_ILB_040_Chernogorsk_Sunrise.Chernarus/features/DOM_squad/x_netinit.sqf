...
if (!isDedicated) then {
	[QGVAR(grpswmsg), {player globalChat ((_this select 1) + " " + localize "STR_AAS_has_joined")}] call FUNC(NetAddEventSTO);
	[QGVAR(grpswmsgn), {player globalChat ((_this select 1) + " " +  localize "STR_AAS_has_left")}] call FUNC(NetAddEventSTO);
};