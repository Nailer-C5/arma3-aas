...
_fortifyorig= false;
_rlastPos = [0,0,0];
//DEFINE !R'S CALLS AND SPAWNS FUNCTIONS (remove those Undefined variable in expression errors)
R_relPos3D = {};
R_ThrowSmoke = {};
...
if (count [_npc,2] call MON_nearestSoldiers > 0) then {
_npc = [[_npc,2] call MON_nearestSoldiers select 0,units ([_npc,2] call MON_nearestSoldiers select 0)] call MON_getleader;
};
};
} else {
...
if (_nowpType == 1) then {
null = [_npc] call R_FN_deleteObsoleteWaypoints;
_nowp = false;
};
};
...
_speedmode = "FULL";
_wpformation = "WEDGE";
_unitpos = if (_rnd < 90) then {"AUTO"} else {"AUTO"};
_minreact = KRON_UPS_minreact;
...
_Behaviour = "AWARE";
_speedmode = "FULL";
_unitpos = if (_rnd < 60) then {"AUTO"} else {"AUTO"};
_minreact = KRON_UPS_minreact * 2;
...
}else{ [_x,"AUTO"] spawn MON_setUnitPos;};
};
sleep 0.01;
} foreach units _npc;
...
//Search for vehicle
if (((!_gothit && _targetdist >= ( KRON_UPS_searchVehicledist )) || ((random 100) < KRON_UPS_vehicleUsePercentage)) && _isSoldier && !_noveh && KRON_UPS_useVehicles) then {
