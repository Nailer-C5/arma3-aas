...
if (AAS_Params_RecruitAIStatusDialog == 1 && AAS_Params_AISupportMode == SUPPORT_LTE4 or AAS_Params_AISupportMode == SUPPORT_LTE6 or AAS_Params_AISupportMode == SUPPORT_LTE8 or AAS_Params_AISupportMode == SUPPORT_LTE10 or AAS_Params_AISupportMode == SUPPORT_LTE12 or AAS_Params_AISupportMode == SUPPORT_EAST or AAS_Params_AISupportMode == SUPPORT_WEST or AAS_Params_AISupportMode == SUPPORT_ON) then 
	{playableUnits;allUnits;} 
	else {playableUnits;};
      } 
	else {switchableUnits;/*allUnits;*/};
	...
	for "_i" from 0 to 49 do {
		...
		_ctrl ctrlShow true;
		_ctrl ctrlSetText (Localize "STR_AAS_Join");
	...
	_grplockbutidc = 7000;
	{
		...
		if (player == leader _x) then {
			...
			CTRL(_grplockbutidc) ctrlSetText (if (_lockedgr) then {Localize "STR_AAS_Unlock"} else {Localize "STR_AAS_Lock"});	
		};
		...
		_ctrl lbSetCurSel -1;
		{
			...
			if (_x == player) then {
				...
				if (count _unitsar > 1) then {
					_ctrl2 ctrlSetText Localize "STR_AAS_Leave";
				} else {
...
FUNC(squadmgmtlockbuttonclicked) = {
	...
	if (!isNil "_lockedgr" && {_lockedgr}) then {
		...
		_ctrl ctrlSetText Localize "STR_AAS_Lock";
		_ctrl2 ctrlShow false;
	} else {
		_grp setVariable [QGVAR(locked), true];
		_ctrl ctrlSetText Localize "STR_AAS_Unlock";
...
FUNC(squadmgmtlbchanged) = {
	...
	if (group player == _grp && {player == leader _grp}) then {
		...
		if (name player != _lbsel) then {
			CTRL(_button) ctrlSetText Localize "STR_AAS_Remove";
		} else {
			CTRL(_button) ctrlSetText Localize "STR_AAS_Leave";
		};
	};
};
...
FUNC(squadmgmtlblostfocus) = {
	...
	if (ctrlText _buctrl == "Remove") then {
		_buctrl ctrlSetText Localize "STR_AAS_Leave";
	};
};
