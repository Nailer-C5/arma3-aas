...
array _westUnits = awu;
array _eastUnits = aeu;
...
_fUpdateWaypointsOfGroups =
{
	...
	if (!(isNull(leader _currentAIgroup))) then
	{
		...
		_marker = format [Localize "STR_AAS_zone",_attackList select (random((count _attackList) - 1))];
...
AAS_fCheckIsUsableVehicleInTheZone =
{
	...
	_return = false;
	_markerName = format[Localize "STR_AAS_zone",_zone];
					...
					waitUntil
					{
						if ((!(_done)) || (_spawnOnce)) then
						{
							...
							for "_j" from (1) to (AAS_Params_NumberOfAIunitsPerGroup) do
							{
								...
								//888
								spawnPosition= spawnPosition findEmptyPosition [0,SPAWNCAMP_RANDOMISE_BOUND];
								//888
