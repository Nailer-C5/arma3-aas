...
while {true} do {
  ...
  _posx = _pos select 0;
  _posy = _pos select 1;
  if ((attached == 0) && (act == 0) && _truck isKindOf "Truck" && driver _truck == player && ((_tposy <= (_posy)) && (_tposy >= (_posy - 2)))) then 
  {
  ...
  {
  if (_truck isKindOf "Truck") then {
  if ((act == 1) && (!(driver _truck == player) or (_truck distance _arty > 7))) then
    {
